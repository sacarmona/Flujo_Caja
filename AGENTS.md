...
> **Estado de consolidación (rama `consolidacion-cash-flow`):** el código base de esta rama
> proviene del repositorio `cash_flow`, que implementa autenticación por magic-link en vez de
> usuario/contraseña como exige este documento. Esa migración (Fase B del plan de
> consolidación) todavía no se ha ejecutado — no asumir que el auth actual ya cumple esta regla
> hasta que se elimine `EmailLoginToken` y se agregue `passwordHash` a `User`.
